...
export class CustomPaginationComponent {
  ...
  /** Generates pagination array with numbers & ellipsis */
  ...
  /** Returns an array of page numbers for dropdown */
  getPageArray(total: number): number[] {
    const visiblePages = new Set(this.pagination.filter(p => typeof p === 'number'));
    return Array.from({ length: total }, (_, i) => i + 1).filter(p => !visiblePages.has(p));
  }
